...
$i = 0;
  ...
  echo"<script>var materias = new Array();</script>";
	//echo"<script>var myData = new Array();</script>";
	echo"<script>var myData = new Array();</script>";

while($i < $n){
	echo "<script>materias.push('{$materias[$i]}');</script>";
	$sql = mysql_query("select count(*) from bitacoras where carrera = '{$materias[$i]}'");
	$row = mysql_fetch_row($sql);
	settype($row[0],"integer");
	echo"<script>myData.push(['$materias[$i]',$row[0]])</script>";
